...
# agent参数， 因为这个必须放在jar包前面，单独抽一个参数
AGENT_OPTS=(
    "-Dotel.service.name=${APP_NAME}"
    "-Dotel.traces.exporter=otlp"
    "-Dotel.metrics.exporter=otlp"
    "-Dotel.exporter.otlp.protocol=grpc"
    # grpc端口4317, 10.88.1.190:4317
    "-Dotel.exporter.otlp.endpoint=${OLTP_URL}"
...
# 即使参数内部含有空格，也不会被拆分
exec java "${DEFAULT_JVM_OPTS[@]}" "${MODULE_OPTS[@]}" ${VM_OPTIONS:+"${VM_OPTIONS}"} "${AGENT_OPTS[@]}" -jar "${JAR_FILE}"
