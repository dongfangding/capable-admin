...
public class SysMenuService {
    ...
    public void persist(SysMenuCreateRequest request) {
        ...
        if (StringUtils.isNotBlank(request.getComponentName())) {
            final SysMenu byName = sysMenuMapper.findByName(request.getComponentName());
            if (Objects.nonNull(byName) && (!Objects.equals(byName.getMenuId(), request.getMenuId())) || (isUpdate
                ...
                throw new BusinessException(PrettyAdminExceptionCode.MENU_FRAME_PATH_INVALID);
            }
        }
        ...
        if (Objects.equals(Boolean.TRUE, request.getIsFrame()) && !StringExtUtil.isValidHttpScheme(request.getPath())) {
            throw new BusinessException(PrettyAdminExceptionCode.MENU_FRAME_PATH_INVALID);
        }
    ...
    public List<BuildMenuRouteNode> buildUserMenuTree(Long userId) {
        ...
        for (SysMenu menu : userMenuList) {
            ...
            node.setName(StringUtils.defaultIfBlank(menu.getComponentName(), menu.getTitle()));
            // 一级目录需要加斜杠，不然会报警告
            node.setPath((Objects.equals(0L, menu.getPid()) ? "/" : "") + menu.getPath());
            node.setComponent(menu.getComponent());
            if (!menu.getIsFrame()) {
                // 一级菜单
                if (Objects.equals(0L, menu.getPid())) {
                    node.setComponent(StringUtils.defaultString(menu.getComponent(), "Layout"));
                } else if (Objects.equals(0, menu.getType())) {
                    // 如果不是一级菜单，且菜单类型为目录，代表是多级菜单
                    node.setComponent(StringUtils.defaultIfBlank(menu.getComponent(), "ParentView"));
                }
            }
            node.setMeta(MenuMetaVo.of(menu.getTitle(), menu.getIcon(), menu.getCache()));
